...
public class GiftPackServerPackRSocketController
  ...
  Mono<GiftPackQueryReply> onQueryException(GiftPackServerExceptions.Query ignored) {
    return Mono.just(GiftPackQueryReply.newBuilder().setError(GiftPackErrorCode.FAILURE).build());
  }
  ...
  Mono<GiftPackPageReply> onPageException(GiftPackServerExceptions.Page ignored) {
    return Mono.just(GiftPackPageReply.newBuilder().setError(GiftPackErrorCode.FAILURE).build());
  }
}
